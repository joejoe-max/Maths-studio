...
def can_solve(problem) -> float:
    ...
    if domain in {"calculus", "ode"}:
        return 1.0 if not problem_type or problem_type in {'ode', 'series', 'integral', 'laplace_transform', 'derivative', 'calculus_expression'} else 0.75
    return 0.0
...
def _normalize_ode_equation(expr_str: str) -> tuple[str, str]:
    """
    Normalize ODE equation into two sides (lhs, rhs).
    Handles multiple formats:
      - "dy/dx = y + x" → ("dy/dx", "y + x")
      - "y' = y + x" → ("y'", "y + x")
      - "y'' + 2*y' + y = 0" → ("y'' + 2*y' + y", "0")
      - "d^2y/dx^2 + dy/dx = sin(x)" → ("d^2y/dx^2 + dy/dx", "sin(x)")
    """
    if "=" not in expr_str:
        raise ValueError("ODE must contain '=' sign")
...
async def _solve_ode(expr_str: str, params: dict):
    ...
    Supports multiple equation formats for robustness.
    """
    yield _section("ORDINARY DIFFERENTIAL EQUATION")

    t = sp.Symbol("t", real=True)
    func_match = re.search(r"\b([A-Za-z])\s*'", expr_str)
    func_name = func_match.group(1) if func_match else "y"
    dependent = sp.Function(func_name)(t)

    # Step 1: Normalize the equation string
    try:
        lhs_str, rhs_str = _normalize_ode_equation(expr_str)
    except ValueError as e:
        yield {"type": "error", "message": str(e)}
        return

    # Step 2: Comprehensive notation map for all derivative formats
    notation_maps = {
        # Replace higher-order derivatives first (longest first to avoid partial replacements)
        rf"d[3\^]{{0,1}}{func_name}\s*\/\s*d[tx][3\^]{{0,1}}": f"Derivative({func_name},t,3)",
        rf"d[2\^]{{0,1}}{func_name}\s*\/\s*d[tx][2\^]{{0,1}}": f"Derivative({func_name},t,2)",
        rf"d{func_name}\s*\/\s*d[tx]": f"Derivative({func_name},t)",
        rf"d\^3{func_name}\s*\/\s*dt\^3": f"Derivative({func_name},t,3)",
        rf"d\^2{func_name}\s*\/\s*dt\^2": f"Derivative({func_name},t,2)",
        rf"d\^2{func_name}\s*\/\s*dx\^2": f"Derivative({func_name},t,2)",
        rf"d{func_name}\s*\/\s*dt": f"Derivative({func_name},t)",
        rf"d{func_name}\s*\/\s*dx": f"Derivative({func_name},t)",
        rf"{func_name}\'\'\'": f"Derivative({func_name},t,3)",
        rf"{func_name}\'\'": f"Derivative({func_name},t,2)",
        rf"{func_name}\'": f"Derivative({func_name},t)",
    }

    # Apply notation normalization to both sides
    for pattern, replacement in notation_maps.items():
        lhs_str = re.sub(pattern, replacement, lhs_str, flags=re.IGNORECASE)
        rhs_str = re.sub(pattern, replacement, rhs_str, flags=re.IGNORECASE)

    def insert_implicit_ode_multiplication(value: str) -> str:
        value = re.sub(r"(?<=\d)\s*(?=Derivative\()", "*", value)
        value = re.sub(rf"(?<=\d)\s*(?={func_name}\b)", "*", value)
        return value

    lhs_str = insert_implicit_ode_multiplication(lhs_str)
    rhs_str = insert_implicit_ode_multiplication(rhs_str)

    try:
        # Step 3: Parse both sides
        local_symbols = {func_name: dependent, "t": t, "Derivative": sp.Derivative}
        lhs_sym = sp.sympify(lhs_str, locals=local_symbols)
        rhs_sym = sp.sympify(rhs_str, locals=local_symbols)
        ...
        # Step 4: Solve using SymPy's dsolve
        sol = sp.dsolve(ode_eq, dependent)
        yield _step(
            "solve_ode",
            "Find the general solution",
            sp.latex(ode_eq),
            sp.latex(sol),
            "Using SymPy's ODE solver (integrating factors / characteristic equation / Laplace method)",
        )
        ...
        yield {
            ...
            "summary": [{"label": f"{func_name}(t)", "value": str(sol.rhs)}],
        }
    except Exception as exc:
        logger.exception(f"ODE solve failed: {exc}")
        yield {
            "type": "error",
            "message": f"Could not solve ODE: {exc}\n\nExpected formats:\n"
                      f"  • dy/dx = y + x\n"
                      f"  • y' = y + x\n"
                      f"  • y'' + 2*y' + y = 0\n"
                      f"  • d²y/dt² + dy/dt = sin(t)"
        }
