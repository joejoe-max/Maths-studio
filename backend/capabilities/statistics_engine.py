...
def can_solve(problem) -> float:
    ...
    if domain == "statistics":
        return 1.0 if not problem_type or problem_type in {'linear_regression', 'hypothesis_test', 'descriptive_statistics'} else 0.75
    return 0.0
...
async def solve_statistics(data):
    ...
    if arr.size == 0:
        yield {"type": "error",
               "message": ("The data list is empty or could not be parsed. "
    ...
    try:
        ...
        yield {"type": "error", "message": f"Statistics solver error: {exc}"}
