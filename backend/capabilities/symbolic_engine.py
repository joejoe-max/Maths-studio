    ...
    try:
        if equations_raw:
            if len(equations_raw) == 1:
                async for evt in _solve_single_equation(equations_raw[0], problem_type):
                    yield evt
            else:
            ...
            if any(kw in problem_type for kw in ("quadratic", "polynomial", "roots")):
                ...
                async for evt in _solve_single_equation(expression_raw, problem_type):
                    yield evt
...
async def _solve_single_equation(eq_str: str, problem_type: str):
    """
    ...
    # Detect variables
    syms = sorted(re.findall(r"\b([a-zA-Z])\b", eq_str))
    syms = list(dict.fromkeys(s for s in syms if s not in ("e",)))
    if not syms:
    ...
    if "=" in eq_str:
        ...
        lhs = _parse(lhs_s.strip(), sym_dict)
        rhs = _parse(rhs_s.strip(), sym_dict)
    else:
        # Treat as expression = 0
        lhs = _parse(eq_str, sym_dict)
        rhs = sp.Integer(0)
...
async def _linear_steps(expr: sp.Expr, x: sp.Symbol, sym_dict: dict):
    ...
    solution = sp.Rational(-b, a) if (sp.Rational(a) != 0) else sp.simplify(-b / a)
    ...
    decimal = float(solution.evalf())
    yield {
        "type": "final",
        "answer": f"### Solution\n\n$$\n{_latex(x)} = {_latex(solution)} = {decimal:.6g}\n$$",
        "summary": [{"label": str(x), "value": str(solution), "decimal": decimal}],
    }
...
async def _solve_system(equations: list[str]):
    ...
    for eq_str in equations:
        for sym in re.findall(r"\b([a-zA-Z])\b", eq_str):
            if sym not in ("e",) and sym not in all_vars:
                all_vars.append(sym)

    sym_dict = {s: sp.Symbol(s) for s in all_vars}
    ...
    for eq_str in equations:
        ...
        if "=" in eq_str:
            ...
            expr = _parse(eq_str, sym_dict)
            parsed_eqs.append(sp.Eq(expr, 0))
            eq_latexes.append(f"{_latex(expr)} = 0")
    ...
    if not solution:
        ...
        try:
            ...
            for eq_str in equations:
                if "=" in eq_str:
                    l, r = eq_str.split("=", 1)
                    lhs_exprs.append(_parse(l.strip(), sym_dict))
                    rhs_vals.append(_parse(r.strip(), sym_dict))
                else:
                    lhs_exprs.append(_parse(eq_str, sym_dict))
                    rhs_vals.append(sp.Integer(0))
    ...
    # Show linear combination steps for 2-variable systems
    if len(all_vars) == 2 and n == 2:
    ...
    for sym_key, sym_obj in sym_dict.items():
        if sym_obj in sol:
            ...
            yield _eq_state(f"{_latex(sym_obj)} = {_latex(val)}", f"Solution for {sym_key}")
            try:
                dec = float(val.evalf())
                summary.append({"label": sym_key, "value": str(val), "decimal": dec})
                result_parts.append(f"$$\n{_latex(sym_obj)} = {_latex(val)} = {dec:.6g}\n$$")
            except Exception:
                summary.append({"label": sym_key, "value": str(val)})
                result_parts.append(f"$$\n{_latex(sym_obj)} = {_latex(val)}\n$$")
