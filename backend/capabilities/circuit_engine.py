...
def can_solve(problem) -> float:
    ...
    if domain == "circuits":
        return 1.0 if not problem_type or problem_type in {'rl_circuit', 'ohms_law', 'circuit_analysis', 'rc_circuit'} else 0.75
    return 0.0
...
async def _ohms_law(params: dict):
    ...
    if len(known) < 2:
        yield {
            "type": "final",
            "answer": (
                "### Ohm's Law — General Form\n\n"
                "Provide any two of `V`, `I`, and `R` for a numeric result.\n"
                "- Voltage: `V = I R`\n"
                "- Current: `I = V / R`\n"
                "- Resistance: `R = V / I`\n"
