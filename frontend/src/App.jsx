...
export default function App() {
  ...
  const [inputPopup, setInputPopup] = useState({
    isOpen: false, fields: [], problemDescription: '',
  });
  ...
  const askForMethod = (methods, domain, query) => {
  ...
  const askForMissingInputs = (payload) => {
    if (!payload?.fields?.length) return Promise.resolve(null);
    return new Promise((resolve) => {
  ...
  const handleInputSubmit = (values, extraEquations = []) => {
    const merged = { ...(values || {}) };
    if (extraEquations.length > 0) {
      const typedEquations = Object.entries(values || {})
        .filter(([key, value]) => key.startsWith('equation_') && String(value || '').trim())
        .sort(([a], [b]) => a.localeCompare(b))
        .map(([, value]) => value);
      merged.equations = [...typedEquations, ...extraEquations.filter(Boolean)];
    }
    inputResolveRef.current?.(merged);
    inputResolveRef.current = null;
    setInputPopup(s => ({ ...s, isOpen: false }));
  };

  const handleInputCancel = () => {
    inputResolveRef.current?.(false);
    inputResolveRef.current = null;
    setInputPopup(s => ({ ...s, isOpen: false }));
  };
  ...
  const handleCompute = async (attachment = null) => {
    if ((!inputText.trim() && !attachment) || isProcessing) return;
    const query = inputText.trim() || `Uploaded file: ${attachment?.name || 'attachment'}`;
    setInputText('');
    await handleComputeQuery(query, { attachment });
  };

  const handleComputeQuery = async (query, options = {}) => {
    ...
    const ts = existingEntryId ? Date.now() : Date.now();
    const entryId = existingEntryId || ts.toString();
    ...
    setEntries(prev => {
      if (!existingEntryId) return [...prev, nextEntry];
      return prev.map(e => (e.id === existingEntryId ? { ...e, ...nextEntry } : e));
    });
    ...
    const payload = {
      type: hasAttachment
        ? (attachment.type?.startsWith('image/') ? 'image' : 'document')
        : 'text',
      input: hasAttachment ? attachment.data : query,
      text: hasAttachment && !query.startsWith('Uploaded file:') ? query : '',
    ...
    try {
      ...
      const endpoint = `${backendBaseUrl}/api/compute/solve`;
      ...
      while (true) {
        ...
        for (const part of parts) {
          ...
          if (data.type === 'needs_parameters') {
            interruption = { type: 'parameters', payload: data };
            setEntries(prev => prev.map(e =>
              e.id === entryId
                ? { ...e, isProcessing: false, error: null }
                : e
            ));
            break streamLoop;
          }

          if (data.type === 'needs_method_selection') {
            interruption = { type: 'method', payload: data };
            setEntries(prev => prev.map(e =>
              e.id === entryId
                ? { ...e, isProcessing: false, error: null }
                : e
      ...
      if (interruption?.type === 'parameters') {
        const supplied = await askForMissingInputs(interruption.payload);
        if (supplied === false) {
          setEntries(prev => prev.map(e =>
            e.id === entryId
              ? { ...e, error: 'Calculation cancelled while waiting for more inputs.', isProcessing: false }
              : e
          ));
          setIsProcessing(false);
          return;
        ...
        await handleComputeQuery(query, {
          entryId,
          supplementalParams: { ...(supplementalParams || {}), ...(supplied || {}) },
          attachment,
        });
        return;
      }

      if (interruption?.type === 'method') {
        const selected = await askForMethod(
          interruption.payload.methods,
          interruption.payload.domain,
          query
        );
        if (selected === false) {
          setEntries(prev => prev.map(e =>
            e.id === entryId
              ? { ...e, error: 'Calculation cancelled while waiting for method selection.', isProcessing: false }
              : e
          ));
          setIsProcessing(false);
          return;
        ...
        setIsProcessing(false);
        const selectedMethodId = typeof selected === 'string' ? selected : selected?.id;
        await handleComputeQuery(query, {
          entryId,
          supplementalParams: { ...(supplementalParams || {}), method: selectedMethodId || undefined },
          attachment,
        });
        return;
      }
    ...
    <div className="h-screen bg-[#08080f] text-slate-200 flex flex-col font-sans overflow-hidden">
      ...
      <header className="h-14 flex items-center justify-between px-3 sm:px-6 border-b border-[#1d1e2c] bg-[#0a0a12]/90 backdrop-blur-md z-50 shrink-0">
        ...
        <div className="flex items-center gap-1 shrink-0">
          ...
          <button onClick={() => setShowHistory(!showHistory)} className={`p-2 rounded hover:bg-white/5 transition-colors ${showHistory ? 'text-blue-400' : 'text-slate-500 hover:text-slate-300'}`} title="Show history">
      ...
      <AnimatePresence>
        {inputPopup.isOpen && (
          <InputPopup
            ...
            problemDescription={inputPopup.problemDescription}
